...
const ItemForm: React.FC<ItemFormProps> = ({ isOpen, onDidDismiss, item, onSave }) => {
  ...
  const [alertHeader, setAlertHeader] = useState('Validation Error');
  ...
  const handleSave = () => {
    ...
    const saveResult = ItemService.saveItem(itemToSave);
    ...
    if (saveResult.success) {
      ...
      setAlertHeader('Storage Error');
      setAlertMessage(saveResult.error || 'Failed to save item due to storage limitations. Try removing some images or deleting unused items.');
      setShowAlert(true);
    }
  };
          ...
          alertHeader === 'Storage Error' 
            ? [
                ...
                },
                {
                  text: 'Storage Info',
                  handler: () => {
                    const usage = ItemService.getStorageUsage();
                    setAlertHeader('Storage Information');
                    setAlertMessage(
                      `Current storage usage: ${usage.usedMB.toFixed(2)}MB of ${usage.totalMB.toFixed(2)}MB (${usage.percentUsed.toFixed(2)}%)\n\n` +
                      `Available space: ${usage.availableMB.toFixed(2)}MB\n\n` +
                      `To free up space, try:\n` +
                      `• Deleting unused items\n` +
                      `• Using smaller images\n` +
                      `• Reducing the number of images per item`
                    );
                    setShowAlert(true);
                  }
                }
